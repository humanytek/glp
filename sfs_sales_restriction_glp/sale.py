...
class sale_order(osv.osv):
    ...
    def action_wait(self, cr, uid, ids, context=None):
        group_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'sfs_sales_restriction_glp', 'group_sale_stock_zero_users')[1]
        ...
        for sale_obj in self.browse(cr, uid, ids, context=context):
            for line in sale_obj.order_line:
                if line.product_id and line.product_id.qty_available == 0 and group_id not in group_ids:
                    raise osv.except_osv(_('UserError'),('''You Must belong to group "Approve Sale Order with real stock 0" in order to confirm this Sale Order'''))
...
class sale_order_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        group_sale_stock_zero_user_ids = [x.id for x in group.users if group.users]
        if uom:
        ...
        if product:
            ...
            if (product_obj.type == 'product') and int(compare_qty) == -1 \
              and (product_obj.procure_method == 'make_to_stock'):
                ...
                if warning_msgs:
                    if not group_sale_stock_zero_user_ids or uid not in group_sale_stock_zero_user_ids:
                        res.update({'value': {'name': False, 'product_id': False, 'product_uom_qty': False}})
