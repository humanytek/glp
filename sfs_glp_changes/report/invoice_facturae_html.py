...
class invoice_facturae_html(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_invoice_note': self.get_invoice_note,
            'get_zona': self.get_zona,
            'get_salesperson' : self.get_salesperson,
#             'get_company_promisory': self._get_text_promissory,
        })
        self.taxes = []
    ...
    def _get_text_promissory(self, company, partner, invoice):
        ...
        print eval("{" + company.dict_var + "}"), company.dinamic_text,"+2"*1000
    ...
    def get_partner_bank_ref(self, partner_id):
        ...
        ref = partner_obj.bank_ref
        return ref
    
    def get_zona(self, partner_id):
        partner_obj = self.pool.get('res.partner').browse(self.cr, self.uid, partner_id.id)
        ref = partner_obj.zona_id and partner_obj.zona_id.name
        return ref
    
    def get_salesperson(self, partner_id):
        partner_obj = self.pool.get('res.partner').browse(self.cr, self.uid, partner_id.id)
        ref = partner_obj.user_id and partner_obj.user_id.name
        return ref
    ...
    def get_qty_product(self, invoice_id):
        ...
        for inv_line in invoice.invoice_line:
            uos_qty = uos_qty + inv_line.quantity
        return uos_qty

    def get_invoice_note(self, invoice_id):
        pool = pooler.get_pool(self.cr.dbname)
        invoice_obj = pool.get('account.invoice')
        invoice = invoice_obj.browse(self.cr, self.uid, invoice_id)
        note = invoice.comment or False
        print note,"{}{}"*1000
        return note

webkit_report.WebKitParser('report.sfs.glp.account.invoice.facturae.webkit.inherit1',
            'account.invoice',
            'addons/sfs_glp_changes/report/invoice_facturae_html.mako',
